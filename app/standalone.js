...
const {getBalance, getJackpot, getBidAddressArray} = require('./contract-functions/get-data');
...
const destroy = () => {
    ...
    gameContractActor.methods.owner().call()
    .then((owner) => {
        console.log('owner', owner);
    });
    gameContractActor.methods.destroy().call()
    .then((result) => {
...
const bid = () => {
    gameActor.eth.getAccounts().then((accountArray) => {
        ...
        gameContractActor.methods.owner().call().then((owner) => {
            console.log('owner', owner);
        });        

        gameActor.eth.getGasPrice().then(gasPrice => {
            console.log('gasPrice', gasPrice);
            gameContractActor.methods.bidValue().call().then(bidValue => {
