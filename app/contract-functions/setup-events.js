...
const setupGameEvents = (
  ...
  gameContractListener.events.NewCurrentBidAddress((error, event) => {
    ...
    logger.info(
      `NewCurrentBidAddress : bidAddress:  ${event.returnValues.bidAddress}`
    );
    logger.info(
      `NewCurrentBidAddress : currentJackpotValue:  ${event.returnValues.jackpot}`
    );
    ...
    logger.info(
      `NewCurrentBidAddress : jackpotId:  ${event.returnValues.jackpotId}`
    );
    if (newBidHandler) {
      newBidHandler(error, event);
    }
  });
  ...
  gameContractListener.events.GameEnded((error, event) => {
    ...
    logger.info(
      `GameEnded : winnerAddress:  ${event.returnValues.winnerAddress}`
    );
