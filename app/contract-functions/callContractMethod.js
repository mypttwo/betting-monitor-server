...
const callContractMethod = async (method, gameActor, value) => {
  ...
  try {
    ...
    let txHash;
    try {
      let receipt = await method
        .send(options)
        .once("transactionHash", (hash) => (txHash = hash));
      // await handler(gameActor);
      logger.info(
        `${method._method.name} TransactionReceipt ${txHash} successfuly mined`
      );
    } catch (e) {
      logger.error(
        `${method._method.name} Error:  ${JSON.stringify(e, null, 3)}`
      );
      if (e.message.includes("not mined within 50 blocks")) {
        const handle = setInterval(() => {
          gameActor.web3.eth.getTransactionReceipt(txHash).then((resp) => {
            if (resp != null && resp.blockNumber > 0) {
              ...
              logger.info(
                `${method._method.name} TransactionReceipt ${txHash} mined in block# ${resp.blockNumber}`
              );
            }
          });
        });
        ...
        logger.error(
          `exiting process ${method._method.name} error: ${JSON.stringify(
            e,
            null,
            3
          )}`
